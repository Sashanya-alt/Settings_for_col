...
BOOL MyDialog2::OnInitDialog()
{
  ...
  //  CWnd*  hListView = GetDlgItem(IDC_LIST2);
    ...
    /*std::vector<string> testNames = { "Column 1", "Column 2", "Column 3" };*/
   /* InitAData2(testNames);*/
    ...
    for (int i = 0; i < AData2.sub.size(); i++) {
         
        lvi.iItem = i;
        
      //  strtmp = AData2.Name[i];
       // strItem = (LPCTSTR) strtmp;


        strItem.Format(_T("%s  "), AData2.Name[i]);
        //lvi.iItem--;
        
        lvi.pszText = (LPTSTR)(LPCTSTR)(strItem);
        MyListCtrl.InsertItem(&lvi);
    
        MyListCtrl.SetCheck(i, 1);
